...
bool writeZipFile(const QString &filePath, QVector<ZipEntry> entries, QString *errorMessage)
{
    ...
    for (ZipEntry &entry : entries)
    {
        ...
        if (!writeUInt32(&file, 0x04034B50U)
            ...
            || !writeUInt32(&file, static_cast<quint32>(entry.data.size()))
            || !writeUInt16(&file, static_cast<quint16>(entry.fileName.size()))
            || !writeUInt16(&file, 0)
            || file.write(entry.fileName) != entry.fileName.size()
    ...
    if (!writeUInt32(&file, 0x06054B50U)
        || !writeUInt16(&file, 0)
        || !writeUInt16(&file, 0)
        || !writeUInt16(&file, static_cast<quint16>(entries.size()))
        || !writeUInt16(&file, static_cast<quint16>(entries.size()))
